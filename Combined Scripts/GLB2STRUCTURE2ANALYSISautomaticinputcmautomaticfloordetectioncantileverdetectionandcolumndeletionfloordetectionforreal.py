...
#DETECT SCALE FUNCTION
# This function detects the scale of the model based on the average vertex coordinates.

def detect_scale(vertices):
    sample_coords = [coord for point in vertices[:10] for coord in point]
    avg = sum(map(abs, sample_coords)) / len(sample_coords)
    
    if avg > 1000:     # e.g., 9800 mm → treat as mm
        return 0.001
    elif avg > 10:      # e.g., 980 cm → treat as cm
        return 0.01
    else:               # e.g., already in meters
        return 1.0
...
def convert_glb_to_3dm(glb_path, output_3dm_path):
    ...
    for mesh_idx, gltf_mesh in enumerate(gltf.meshes):
        ...
        for primitive in gltf_mesh.primitives:
            ...
            # Detect unit scale
            scale_factor = detect_scale(vertices)
...
for roof_id, bbox, poly in roof_meshes_info:
    local_max_z = bbox.Max.Z
    num_floors = max(1, int(round(local_max_z / floor_height)))
    roof_floors[str(roof_id)] = {
        "height": round(local_max_z, 2),
        "floors": num_floors
    }

print("=== Floors per roof mesh ===")
for rid, info in roof_floors.items():
    print(f"Roof ID: {rid}, Height: {info['height']}m → {info['floors']} floors")
...
# === GROUP ROOF-COVERED FLOORS BY VOLUME ===
roof_coverage = []

for roof_id, roof_bbox, roof_poly in roof_meshes_info:
    covered_floors = []
    for z, polys in floor_footprints_by_level.items():
        for poly in polys:
            if roof_poly.buffer(0.1).intersects(poly):  # intersection with footprint
                covered_floors.append(z)
    roof_coverage.append({
        "roof_id": str(roof_id),
        "roof_height": roof_bbox.Max.Z,
        "covered_floors": sorted(covered_floors)
    })

# === CALCULATE LOCAL FLOOR STACKS PER VOLUME ===
floor_height = 2.5  # meters per floor (you can adjust)
for entry in roof_coverage:
    roof_z = entry['roof_height']
    min_floor_z = min(entry['covered_floors']) if entry['covered_floors'] else 0.0
    height_under_roof = roof_z - min_floor_z
    num_floors = max(1, int(round(height_under_roof / floor_height)))
    entry['local_floor_z_levels'] = [round(min_floor_z + i * floor_height, 5) for i in range(num_floors + 1)]

print("\n=== Local floor levels under each roof ===")
for e in roof_coverage:
    print(f"Roof ID: {e['roof_id']}, Height: {e['roof_height']} → Floors: {e['local_floor_z_levels']}")
...
node_id_map = {}  # Dictionary to store all 3D nodes
node_counter = [0]  # Use a list to allow modification from inside function

def add_node(coord):
    for node_id, pt in node_id_map.items():
        if all(abs(c1 - c2) < 1e-6 for c1, c2 in zip(pt, coord)):
            return node_id  # Node already exists
    node_id = node_counter[0]
    node_id_map[node_id] = coord
    node_counter[0] += 1
    return node_id
...
for x, y in all_base_columns:
    ...
    # Match the roof volume
    matched_floors = []
    for entry in roof_coverage:
        roof_poly = None
        for rid, bbox, poly in roof_meshes_info:
            if str(rid) == entry['roof_id']:
                roof_poly = poly
                break
        if roof_poly and roof_poly.buffer(0.1).contains(pt):
            matched_floors = entry['local_floor_z_levels']
            break

    if not matched_floors:
        matched_floors = sorted_floor_z_levels  # fallback

    # Filter to local height
    column_z_levels_for_this_column = [z for z in matched_floors if z <= current_column_max_z + 1e-6]

    # If skipping ground floor, filter it out using matched (local) Zs
    if skip_ground_to_first and len(column_z_levels_for_this_column) >= 2:
        local_first_floor_z = matched_floors[1]  # assuming [ground, 1st, 2nd...]
        column_z_levels_for_this_column = [
            z for z in column_z_levels_for_this_column if z >= local_first_floor_z - 1e-6
        ]
        # Make sure we still have at least 2 levels to form a column
        if len(column_z_levels_for_this_column) < 2:
            # Recover with forced start at first floor if column was accidentally wiped
            column_z_levels_for_this_column = [local_first_floor_z, current_column_max_z]

    # Finally, generate column segments
...
# ✅ Get beam levels from actual column node heights
unique_z_levels = sorted(set(round(pt[2], 5) for pt in node_id_map.values()))
print(f"✅ Beam Z levels from column nodes: {unique_z_levels}")

# ✅ Loop through each 2D beam and project it at every valid Z level
for (x1, y1), (x2, y2) in beams_2d_lines:
    ...
    for z in unique_z_levels:
        ...
        # ✅ Only use existing column nodes — don’t add new ones here
        id1 = next(
            (nid for nid, pt in node_id_map.items()
             if all(abs(c1 - c2) < 1e-6 for c1, c2 in zip(pt, coord1))),
            None
        )
        id2 = next(
            (nid for nid, pt in node_id_map.items()
             if all(abs(c1 - c2) < 1e-6 for c1, c2 in zip(pt, coord2))),
            None
        )

        # ✅ Only connect if both nodes exist (e.g. due to column segment at this floor)
        ...
        # Optional debug log:
        # else:
        #     print(f"⚠️ Skipped beam at Z={z:.2f} between ({x1:.2f},{y1:.2f}) and ({x2:.2f},{y2:.2f}) — node(s) missing")
...
# --------------------------------------
# Correct CSV Viewer - Now uses the dynamically generated 3DM file and CSV paths
# --------------------------------------
...
# Removed hardcoded base_github_path and CSV_SAVE_PATH, now using derived paths
# from the dynamic input section above.
# The rhino_path is already set to output_3dm_file_path from the previous section.

print(f"Re-attempting to load CSVs from: {os.path.abspath(EXPORT_SAVE_PATH)}")
