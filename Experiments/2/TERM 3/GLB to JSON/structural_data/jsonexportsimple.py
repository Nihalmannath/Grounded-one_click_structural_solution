...
try:
    ...
    try:
        ...
        print(f"⚠️ Warning: Cannot list buckets: {bucket_err}")
    ...
    print(f"❌ Error accessing bucket or listing files: {e}")
...
try:
    ...
    if upload_response:
        ...
        try:
            ...
            print(f"⚠️ Warning: Could not generate public URL: {url_err}")
        ...
        try:
            ...
            print(f"⚠️ Warning: Could not list files after upload: {list_err}")
    else:
        print("❌ Upload response was empty or failed")
        
except Exception as e:
    print(f"❌ Error uploading to analysis-results bucket: {e}")
    ...
    if "403" in str(e) or "Unauthorized" in str(e) or "row-level security policy" in str(e):
        ...
        try:
            ...
            if alt_upload_response:
                print(f"✅ Successfully uploaded to models bucket as '{alt_json_filename}'")
                upload_successful = True
                
                # Generate a public URL for the uploaded file
                try:
                    alt_public_url = supabase.storage.from_("models").get_public_url(alt_json_filename)
                    print(f"📄 Public URL: {alt_public_url}")
                except Exception as alt_url_err:
                    print(f"⚠️ Warning: Could not generate public URL for alternative upload: {alt_url_err}")
            else:
                print("❌ Alternative upload also failed")
                
        except Exception as alt_err:
            print(f"❌ Alternative upload to models bucket also failed: {alt_err}")
...
if not upload_successful:
    ...
    try:
        ...
        print(f"💾 Saved combined structural data locally: {local_json_path}")
    except Exception as local_err:
        print(f"❌ Error saving locally: {local_err}")
        ...
        try:
            ...
            with open(current_dir_path, 'w') as f:
                f.write(combined_json_data)
            print(f"💾 Saved in current directory as ultimate fallback: {current_dir_path}")
        except Exception as current_dir_err:
            print(f"❌ Critical error: Cannot save file anywhere: {current_dir_err}")
else:
